...
| [p0-acceptance.md](p0-acceptance.md) | P0 验收标准逐条状态与证据出处 | 跟踪中 |
| [distribution.md](distribution.md) | 发布 runbook：cargo-dist 链路、资产命名规范、首个 Release 步骤、cask 接入 | 已产出（待首个 tag 实跑验证） |
...
| 2026-09-25 | **P0 打包分发落地**：cargo-dist 0.33 接入（dist-workspace 五目标矩阵、macOS 双架构 merge 到 macos-14 单 job、资产名 latermd-{triple} 无版本号、bin 改名 latermd）、自建 macOS universal2 dmg job（.github/workflows/macos-dmg.yml，lipo + .app + hdiutil）、cask 模板（packaging/latermd.rb，复刻 lscreen）。新增 [distribution.md](distribution.md) 发布 runbook（首个 Release 步骤、资产核对清单、tap 三处接入点、auto-bump 只管 Formula 的手动空档、命名规范与风险 #7 提醒）；README 安装节补 macOS 直下 Gatekeeper 指引、删 Linuxbrew 误导。P0 剩余 = 首个 Release + 真机验收 |
