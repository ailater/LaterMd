...
- **已知并接受的边界**：`byte_start` 是**渲染文本**（经 wikilink 展开 / heal）内的偏移，而大纲 span 基于**源码**；文档里有 `[[wikilink]]` 时两者会错位（误差等于展开新增的字符数）。heal 对完整文档是恒等变换，故绝大多数文档不受影响；要彻底消除需在预览侧再做一次源码→渲染文本的偏移映射，收益小于成本。跳转粒度是**节**（section），不是精确的标题行 —— 标题所在节的顶部。
