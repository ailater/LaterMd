//! 仓库全文搜索(P1 侧边栏搜索,docs/roadmap.md 阶段 3):后台线程
...
//!   原语;[`SearchState`] 是服务之上的面板状态(输入文本、去抖计时、
//!   结果缓存上限),归约与绘制的分工见其方法文档。
//!
...
/// 结果缓存上限:超出即截断并取消后台搜索(导航场景命中 500 条足够,
/// 再多只是把面板变成不可用的长列表)。
...
mod tests {
    ...
    fn search_state_debounce_and_invalid_pattern() {
        let root = sample_vault("state-machine");
